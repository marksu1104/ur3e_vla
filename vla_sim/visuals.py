"""Render-only materials and mesh presentation for the canonical scene."""
...
def set_plastic_material(
    stage, prim_path: str, color: tuple[float, float, float]
) -> None:
...
def configure_gripper_pads(stage) -> list[str]:
    ...
    if len(bound_paths) != 2:
        raise RuntimeError(
            f"Expected two Robotiq inner-finger links, found {bound_paths}"
        )
    return bound_paths
