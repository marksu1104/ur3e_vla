...
完全沒有副作用 (不 import isaaclab / omni / carb), 所以任何時候都能 import.
sim_r.py / sim_vla.py / sim_ros2.py / 還有測試 script 全部共用這份.
"""
...
DEFAULT_ORBIT_SNAPSHOTS    = 24   # 0 = 不拍環繞快照
DEFAULT_ORBIT_VIDEO_FRAMES = 240  # 240 frames @ 30fps = 8 秒一圈
...
# ── EE workspace clamp (x, y, z) ────────────────────────────────────
# Keep actions inside a safe box around the table to avoid resets.
WORKSPACE_X = (-0.4, 0.7)
WORKSPACE_Y = (-0.5, 0.5)
WORKSPACE_Z = (1.05, 1.65)


# ── Orbit camera trajectory (post-recording) ──────────────────────────
